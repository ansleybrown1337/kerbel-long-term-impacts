# Bayesian-versus-ML comparison v3p3

Active entry point: `code/comparison/bayes_ml_comparison_v3p3_physical_event.py`.

The comparison pairs completed Bayesian `v3p3_physical_event` outputs with
completed ML `v3p3_physical_event` outputs. Both runoff-volume models include
the same reviewed event-level `FurrowTireCompaction` exposure, while neither
adds a direct compaction term or feature to concentration. It consumes
...
Inspect `results/comparison/v3p3_physical_event/run_manifest_comparison_v3p3_physical_event.json`,
raw tables before publication tables, and `figures/comparison/v3p3_physical_event/`.
