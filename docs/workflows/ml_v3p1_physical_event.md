# ML v3p1 physical-event workflow

Active entry point: `code/ml/ml_catboost_conformal_loyo_v3p1_physical_event.py`.
...
Inspect `results/ml/v3p1_physical_event/run_manifest_ml_v3p1_physical_event.json`,
...
audits. Observed annual figures use
replicate-plot completeness: ranges only for two complete plots, a distinct x
marker without an interval for one complete plot, and no point for zero complete
plots. In figures, “95% calibration-residual PI” is a prediction interval for
