# ML v3p3 physical-event workflow

Active entry point: `code/ml/ml_catboost_conformal_loyo_v3p3_physical_event.py`.
...
ML v3p3 adds the reviewed binary `FurrowTireCompaction` predictor to the
runoff-volume (`logV`) feature set only. It is absent from the concentration
(`logC`) feature set. The exposure covers 120 physical events: 2021 ST and
...
Inspect `results/ml/v3p3_physical_event/run_manifest_ml_v3p3_physical_event.json`,
