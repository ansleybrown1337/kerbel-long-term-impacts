# Bayesian v3p2 physical-event workflow

Active files:

- `code/bayes/stir-bayes-load_v3p2_physical_event.Rmd`
- `code/bayes/stir-bayes-load_v3p2_physical_event.R` (primary all-at-once entry point)
- `code/bayes/m_stir_mogp_v3p2_physical_event.stan`
- `code/bayes/stir-bayes-backend.R`

## v3p2 model change

v3p2 preserves the corrected v3p1 data, event identities, latent physical
truths, missingness models, censoring, multivariate concentration innovations,
and reporting rules. It changes two modeling choices:

1. Runoff volume now has one global standardized direct effect on
   concentration, `beta_vol`. The v3p1 hierarchy estimated a shared mean, a
   between-analyte scale, and 10 analyte deviations; its shared scale and all
   10 derived slopes failed the 1.04 R-hat review threshold.
2. Standardized zero-centered regression/location priors that previously used
   SD 0.3 or 0.7 now use `normal(0, 1)`. Exponential scale priors now use rate
   1. Conditional hierarchical priors, likelihood distributions, LKJ
   correlation priors, and the bounded `beta(2, 10)` residue prior retain their
   scientifically distinct forms.

The hierarchical log observation-error location is widened from
`normal(log(0.2), 0.7)` to `normal(log(0.2), 1)`. Its nonzero center is retained
because zero on that log-SD scale would imply a median observation error of one
full standardized outcome SD. The complete prior audit is in
`docs/methods/bayesian_prior_audit_v3p2.md`.

This is a Bayesian-only release. ML and existing comparison outputs remain
v3p1. Because the cleaned input and 528-event definition did not change, the
v3p2 runner deliberately reuses the accepted
`validation/preflight/physical_event_v3p1` data contract while writing every
new Bayesian fit, table, diagnostic, and figure under v3p2 paths.
...
`code/bayes/stir-bayes-load_v3p2_physical_event.R`. From Anaconda Prompt or
PowerShell at the repository root, run:

```powershell
& 'C:\Program Files\R\R-4.4.2\bin\x64\Rscript.exe' code\bayes\stir-bayes-load_v3p2_physical_event.R
```
...
`results/bayes/v3p2_physical_event/overall_diagnostics_bayes_v3p2_physical_event.csv`.
...
`Rscript code/bayes/diagnose_saved_fit_v3p2.R`; it writes parameter, chain,
...
Inspect `results/bayes/v3p2_physical_event/run_manifest_bayes_v3p2_physical_event.json`, sampler diagnostics, row/event prediction diagnostics, event-date audits, the event-analyte ledger, replicate-aware annual draws, observed plot-completeness exports, and `figures/bayes/v3p2_physical_event/` before proceeding.
