// m_stir_mogp_v3p0_physical_event.stan
// v3p0_physical_event (physical-event latent volume; row-level concentration):
// - Uses exactly one latent outflow volume per PhysicalEventID.
// - Maps zero or more genuine volume observations to each physical event.
...
//
// Notes:
// - VOL_obs and C are assumed z-scored upstream (as in the pipeline).
...
data {
  ...
  // Physical plot events. E maps concentration rows to physical events;
  // E_rep_row supplies one row's event-level predictors to the volume process.
  int<lower=1> E_n;
  array[N] int<lower=1, upper=E_n> E;
  array[E_n] int<lower=1, upper=N> E_rep_row;

  // Genuine observed-volume rows. Events with missing volume have no row here;
  // confirmed zero runoff remains a valid value in VOL_obs.
  int<lower=0> J_VOL;
...
// ---------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
  // OBSERVATION models (common measurement error) for genuine volume rows and C.
  // ---------------------------------------------------------------------------
  for (j in 1:J_VOL) {
    target += normal_lpdf(VOL_obs[j] | V_true_event[VOL_event_id[j]], sigma_VOL_obs);
  }
...
generated quantities {
  ...
  for (j in 1:J_VOL) {
    VOL_obs_rep[j] = normal_rng(V_true_event[VOL_event_id[j]], sigma_VOL_obs);
  }
