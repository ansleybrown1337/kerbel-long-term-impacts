...
"""Physical-event v3p3 CatBoost workflow for the Kerbel monitoring record.
...
Default inputs and outputs remain compatible with the original workflow:
    ...
    output: results/ml/v3p3_physical_event/
    figures: figures/ml/v3p3_physical_event/
"""
...
CONFIG_PATH = (
    Path(__file__).resolve().parents[2] / "config" / "physical_event_v3p3.json"
)
if not CONFIG_PATH.is_file():
    raise FileNotFoundError(f"ML v3p3 configuration is absent: {CONFIG_PATH}")
...
def find_repo_root(start: Path) -> Path:
    ...
    for _ in range(15):
        if (
            ...
            and (cur / "code").is_dir()
            and (cur / "config" / "physical_event_v3p3.json").is_file()
        ):
            return cur
        if cur.parent == cur:
            break
    ...
    raise FileNotFoundError(
        ...
        "config/physical_event_v3p3.json. "
...
def attach_furrow_tire_compaction(
    ...
    The exposure is deliberately attached only to the physical-event table
    used by the runoff-volume model. Concentration rows never receive this
    feature, preventing a direct concentration path.
    """
...
def select_features(
    ...
    volume_desired = [
        ...
        "DaysSincePlant", "DaysUntilHarvest", "FurrowTireCompaction",
    ]
    if previous_crop:
...
def fit_conformal_model(
    ...
    if calibration != "physical-event":
        raise ValueError("v3p3 requires --calibration physical-event.")
...
def save_model(
    ...
    metadata = {
        ...
        "furrow_tire_compaction_feature_scope": "runoff_volume_only",
        "uses_furrow_tire_compaction": (
            target == "logV" and "FurrowTireCompaction" in features
        ),
        "saved_utc": dt.datetime.now(dt.timezone.utc).isoformat(timespec="seconds"),
        "workflow": "ml_catboost_conformal_loyo_v3p3_physical_event.py",
    }
...
def generate_postprocess_figure_suite(
    ...
    from ml.ml_postprocess_plots_v3p3_physical_event import generate_figures
...
def update_existing_run_manifest(output_dir: Path, **fields: object) -> None:
    ...
    manifest_path = output_dir / "run_manifest_ml_v3p3_physical_event.json"
...
def main() -> None:
    ...
    if "FurrowTireCompaction" in features_c:
        raise ValueError(
            "FurrowTireCompaction must not enter the concentration model."
        )
    ...
    events.loc[events["y_logV"].notna()].to_csv(
        output_dir / "volume_observation_training_table_v3p3.csv", index=False
    )
    concentration_rows.loc[concentration_rows["y_logC"].notna()].to_csv(
        output_dir / "concentration_observation_training_table_v3p3.csv", index=False
    )
    ...
    compaction_event_audit.to_csv(
        output_dir
        / "tire_compaction_event_audit_ml_v3p3_physical_event.csv",
        index=False,
    )
    compaction_audit_summary.to_csv(
        output_dir
        / "tire_compaction_audit_summary_ml_v3p3_physical_event.csv",
        index=False,
    )
    ...
    summary_audit = pd.DataFrame(
        [
            ...
            (
                ...
                "binary predictor used in runoff volume only",
            ),
            (
                "compacted_events_with_genuine_volume",
                int((
    ...
    print(
        ...
        "with genuine volume; included in logV only."
    )
    if args.preflight_only:
        print(
            "[v3p3 ML] PRE-FLIGHT PASSED. The 528-event contract and "
            "runoff-volume-only compaction feature are ready; model fitting "
    ...
    event_ledger.to_csv(output_dir / "event_analyte_draw_ledger_ml_v3p3.csv", index=False)
    event_point_ledger.to_csv(
        output_dir / "event_analyte_point_ledger_ml_v3p3.csv", index=False
    )
    ...
    (output_dir / "run_manifest_ml_v3p3_physical_event.json").write_text(
        json.dumps(manifest, indent=2) + "\n", encoding="utf-8"
    )
    if args.figures:
