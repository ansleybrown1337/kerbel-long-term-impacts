...
"""Physical-event v3p0 CatBoost workflow for the Kerbel monitoring record.
...
Default inputs and outputs remain compatible with the original workflow:
    ...
    output: results/ml/v3p0_physical_event/
    figures: figures/ml/v3p0_physical_event/
"""
...
def find_repo_root(start: Path) -> Path:
    ...
    for _ in range(15):
        if (
            ...
            and (cur / "code").is_dir()
            and (cur / "config" / "physical_event_v3p0.json").is_file()
        ):
            return cur
        if cur.parent == cur:
            break
...
def build_physical_event_tables(
    ...
    for column in ["MeasureMethod", "FlumeMethod", "source_row_count", "_confirmed_zero"]:
        if column not in absent:
            absent[column] = pd.NA
    volume_rows = pd.concat([volume_rows, absent.reindex(columns=volume_rows.columns)], ignore_index=True)
...
def fit_conformal_model(
    ...
    if calibration != "physical-event":
        raise ValueError("v3p0 requires --calibration physical-event.")
...
def point_load_products(
    ...
    *,
    ...
    concentration_groups = [
        "PhysicalEventID", "Year", "Treatment", analyte_column,
    ]
    ...
    ledger = ledger.merge(
        c_audit,
        on=["PhysicalEventID", "Year", "Treatment", "Analyte"],
        how="left",
        validate="one_to_one",
    ).merge(
        v_audit,
    ...
    annual = (
        ...
        .agg(
            AnnualLoad_kg=("Load_kg", "sum"),
            n_physical_events=("PhysicalEventID", "nunique"),
        )
    )
    annual["AnnualLoad_mg"] = annual["AnnualLoad_kg"] * 1_000_000.0
    annual["central_estimate_type"] = "sum_of_physical_event_point_loads"
    audit_columns = [
        column for column in ledger.columns
        if column in {"PhysicalEventID", "Year", "Treatment", "Analyte"}
...
def save_model(
    ...
    metadata = {
        ...
        "workflow": "ml_catboost_conformal_loyo_v3p0_physical_event.py",
    }
...
def full_record_annual_products(
    ...
    *,
    ...
    ``use_observed=False`` is the model-only reconstruction. ``True`` is the
    explicitly labeled observed-plus-imputed sensitivity. In either case all
    predictions exist at row/observation level before within-draw resolution.
    """

    if draws <= 0:
        raise ValueError("Full-record reconstruction requires at least one draw.")
    ...
    c_resolved = resolve_prediction_draws(
        c_long,
        group_columns=["PhysicalEventID", "Year", "Treatment", analyte_column, "Draw"],
        value_column="Concentration_mg_L",
        method=concentration_resolution,
    ...
    annual_draws = (
        ledger.groupby(["Year", "Treatment", "Analyte", "Draw"], as_index=False)
        .agg(AnnualLoad_mg=("AnnualLoad_mg", "sum"))
    )
...
def refit_and_impute(
    ...
    imputed.to_csv(output_dir / "row_values_observed_plus_imputed_sensitivity.csv", index=False)
    imputed[[
        column for column in [
    ...
    sensitivity_point_ledger, sensitivity_point_annual, sensitivity_point_audit = (
    ...
    # Write the two very large draw ledgers sequentially so they are never both
    # retained in memory.
    ...
    sensitivity_ledger, sensitivity_summary = full_record_annual_products(
        imputed, event_predictions, int(args.impute_draws), float(args.alpha), args.seed + 90001,
        use_observed=True,
        concentration_resolution=args.concentration_resolution,
        volume_resolution=args.volume_resolution,
        method_priority=args.method_priority,
    ...
    if sensitivity_summary["point_total_mg"].isna().any():
        raise AssertionError("An observed-plus-imputed annual point total is missing.")
    sensitivity_ledger.to_csv(
        output_dir / "event_analyte_draw_ledger_observed_plus_imputed_sensitivity.csv", index=False
    )
    sensitivity_summary.to_csv(
        output_dir / "annual_load_summary_observed_plus_imputed_sensitivity.csv", index=False
    )
...
def generate_postprocess_figure_suite(
    ...
    from ml.ml_postprocess_plots_v3p0_physical_event import generate_figures
    ...
    generate_figures(
        ...
        use_imputed=bool(args.impute_missing),
    )
...
def main() -> None:
    ...
    preflight_metadata_path = repo / "validation" / "preflight" / "physical_event_v3p0" / "preflight_metadata.json"
    ...
    if (
        preflight_metadata.get("workflow_version") != CORRECTED_VERSION
    ...
    events.loc[events["y_logV"].notna()].to_csv(
        output_dir / "volume_observation_training_table_v3p0.csv", index=False
    )
    concentration_rows.loc[concentration_rows["y_logC"].notna()].to_csv(
        output_dir / "concentration_observation_training_table_v3p0.csv", index=False
    )
    ...
    summary_audit = pd.DataFrame(
        [
            ("workflow_version", CORRECTED_VERSION, "corrected ML workflow"),
    ...
    for held_year in progress:
        ...
        if c_test.any():
            ...
            c_load = concentration_rows.loc[c_test, [
                "PhysicalEventID", "Year", "Treatment", analysis_analyte_column,
        ...
        if paired_events and int(args.draws) > 0:
            ...
            c_resolved = resolve_prediction_draws(
                c_long,
                group_columns=["PhysicalEventID", "Year", "Treatment", "Analyte", "Draw"],
                value_column="Concentration_mg_L",
    ...
    event_ledger.to_csv(output_dir / "event_analyte_draw_ledger_ml_v3p0.csv", index=False)
    event_point_ledger.to_csv(
        output_dir / "event_analyte_point_ledger_ml_v3p0.csv", index=False
    )
    ...
    manifest = {
        ...
        "primary_ml_central_estimate": "sum_of_physical_event_point_loads",
    ...
    (output_dir / "run_manifest_ml_v3p0_physical_event.json").write_text(
