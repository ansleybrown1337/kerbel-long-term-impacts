...
"""Post-process the Kerbel v3p0 physical-event ML workflow.
...
def plot_feature_importance(path: Path, figure_dir: Path, target: str, top_k: int) -> None:
    ...
    axis.set_xlabel("CatBoost importance (mean ± SD across LOYO folds)")
...
def observed_annual_loads(source_path: Path) -> pd.DataFrame:
    ...
    ledger = concentration_event.merge(
        volume_event, on=["PhysicalEventID", "Draw"], how="inner", validate="many_to_one"
    ).merge(
        concentration_counts, on=["PhysicalEventID", "Analyte"],
        how="left", validate="one_to_one",
    )
    ledger["event_load_mg"] = ledger["Concentration_mg_L"] * ledger["Volume_L"]
    return ledger.groupby(["Year", "Treatment", "Analyte"], as_index=False).agg(
        observed_load_mg=("event_load_mg", "sum"),
        observed_rows=("observed_rows", "sum"),
        observed_physical_events=("PhysicalEventID", "nunique"),
    )
...
def plot_annual_loads(
    ...
    for analyte in selected:
        ...
        axis.set_title(
            ...
            "ML line: physical-event point total; band: 95% calibration-residual PI"
        )
...
def plot_event_unit_audit(output_dir: Path, figure_dir: Path) -> None:
    ...
    training_path = output_dir / "volume_observation_training_table_v3p0.csv"
...
def generate_figures(
    *,
    ...
    print("[DONE] v3p0 physical-event post-processing figures created.")
