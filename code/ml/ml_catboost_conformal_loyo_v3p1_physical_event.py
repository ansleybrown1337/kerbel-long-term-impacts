...
"""Physical-event v3p1 CatBoost workflow for the Kerbel monitoring record.
...
Default inputs and outputs remain compatible with the original workflow:
    ...
    output: results/ml/v3p1_physical_event/
    figures: figures/ml/v3p1_physical_event/
"""
...
def find_repo_root(start: Path) -> Path:
    ...
    for _ in range(15):
        if (
            ...
            and (cur / "code").is_dir()
            and (cur / "config" / "physical_event_v3p1.json").is_file()
        ):
            return cur
        if cur.parent == cur:
            break
...
def fit_conformal_model(
    ...
    if calibration != "physical-event":
        raise ValueError("v3p1 requires --calibration physical-event.")
...
def save_model(
    ...
    metadata = {
        ...
        "workflow": "ml_catboost_conformal_loyo_v3p1_physical_event.py",
    }
...
def generate_postprocess_figure_suite(
    ...
    from ml.ml_postprocess_plots_v3p1_physical_event import generate_figures
...
def update_existing_run_manifest(output_dir: Path, **fields: object) -> None:
    ...
    manifest_path = output_dir / "run_manifest_ml_v3p1_physical_event.json"
...
def main() -> None:
    ...
    preflight_metadata_path = repo / "validation" / "preflight" / "physical_event_v3p1" / "preflight_metadata.json"
    ...
    events.loc[events["y_logV"].notna()].to_csv(
        output_dir / "volume_observation_training_table_v3p1.csv", index=False
    )
    concentration_rows.loc[concentration_rows["y_logC"].notna()].to_csv(
        output_dir / "concentration_observation_training_table_v3p1.csv", index=False
    )
    ...
    event_ledger.to_csv(output_dir / "event_analyte_draw_ledger_ml_v3p1.csv", index=False)
    event_point_ledger.to_csv(
        output_dir / "event_analyte_point_ledger_ml_v3p1.csv", index=False
    )
    ...
    (output_dir / "run_manifest_ml_v3p1_physical_event.json").write_text(
        json.dumps(manifest, indent=2) + "\n", encoding="utf-8"
    )
    if args.figures:
