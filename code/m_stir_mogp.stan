...
data {
    ...
    vector[N] VOL;
    array[1637] int VOL_missidx;

    vector[N] STIR;
    vector[N] CIN;
    array[560] int CIN_missidx;
...
parameters {
    ...
    // missing data
    vector[1637] VOL_impute;
    vector[560] CIN_impute;
}
...
transformed parameters {
    ...
    // non-centered MVNs for existing random effects
...
model {
    ...
    for (i in 1:N) {
        mu_C[i] =
            ...
            gamma_S[A[i], S[i]] +
            gamma_F[A[i], Fu[i]] +
            F_year[Y[i], A[i]];   // analyte-specific year effect from multi-output GP
    }
