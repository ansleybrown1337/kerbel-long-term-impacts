// m_stir_mogp_v1p9.stan
// v1p9 (positive-support outcome version built from v1p8):
...
//
// Notes:
// - VOL and C are assumed z-scored upstream on the LOG scale: log(Volume) and log(Concentration).
...
data {
  ...
  // Concentration censoring (left-censor at reporting limit on SAME z-scored LOG scale as C)
...
vector[A_n] z_beta_irr;
  ...
  // process residual scales (latent truth variability on z-scored LOG scale)
  real<lower=0> sigma_analyte;  // global process SD for C_true around mu_C (z-scored log concentration)
  real<lower=0> sigma_V;               // process SD for V_true around mu_V (z-scored log volume)
...
model {
  ...
  // concentration measurement SD (z-scored log scale): hierarchical, analyte-specific
...
// ---------------------------------------------------------------------------
  // Latent PROCESS model for volume truth (z-scored log scale), non-centered
  // V_true is defined in transformed parameters as: V_true = mu_V + sigma_V * z_V
  // ---------------------------------------------------------------------------
  z_V ~ std_normal();
  // ---------------------------------------------------------------------------
  // Latent PROCESS model for concentration truth (z-scored log scale), non-centered
  ...
  // ---------------------------------------------------------------------------
  // OBSERVATION models (measurement error) for VOL and C (z-scored log scale)
