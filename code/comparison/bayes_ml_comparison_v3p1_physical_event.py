#!/usr/bin/env python3
"""Compare only completed Bayesian and ML v3p1 physical-event outputs.
...
def ml_full_record_point_diagnostics(ml_dir: Path) -> pd.DataFrame:
    ...
    concentration = read_required(
        ml_dir / "concentration_observation_training_table_v3p1.csv",
    ...
    volume = read_required(
        ml_dir / "volume_observation_training_table_v3p1.csv",
...
def build_performance_products(
    ...
    bayes_rows = read_required(
        bayes_dir / "row_prediction_diagnostics_bayes_v3p1_physical_event.csv",
        "Bayesian row predictions",
    )
    bayes_volume = read_required(
        bayes_dir / "volume_prediction_diagnostics_bayes_v3p1_physical_event.csv",
...
def update_saved_comparison_manifest(output_dir: Path) -> None:
    ...
    path = output_dir / "run_manifest_comparison_v3p1_physical_event.json"
    if not path.is_file():
        return
...
def annual_volume_comparison(bayes_dir: Path, ml_dir: Path) -> pd.DataFrame:
    ...
    bayes_sources = [
        (
            ...
            "model_only",
            read_required(
                bayes_dir / "annual_volume_kL_wide_modeled_v3p1_physical_event.csv",
...
def plot_observed_replicate_points(
    ...
    *,
    ...
    """Plot n=2 values with replicate ranges and n=1 values as x markers."""
    ...
    if not single.empty:
        axis.scatter(
            ...
            marker="x", color="black", s=55, linewidths=1.6, zorder=10,
        )
...
def plot_annual_comparison(
    ...
    legend_handles = [
        ...
        Line2D(
            ...
            label="ML mean of replicate annual plot totals",
        ),
        Line2D(
            [0], [0], color="black", marker="o", markerfacecolor="none",
            linestyle="none", markersize=8,
            label="Observed annual load (replicate range; x = one complete plot)",
        ),
    ]
    for analyte, group in primary.groupby("Analyte", sort=False):
        ...
        save_figure(
            ...
            figure_dir / "annual_obs_vs_modeled" / f"annual_load_{safe}_obs_vs_modeled_v3p1",
            figure_dir / "annual_complete_observed"
            / f"annual_load_{safe}_complete_observed_v3p1",
            layout_top=0.88,
        )
...
def plot_annual_volume_comparison(volume: pd.DataFrame, figure_dir: Path) -> None:
    ...
    legend_handles = [
        ...
        Line2D(
            [0], [0], color=colors["ML"], marker="o", linestyle="--", linewidth=2,
            label="ML mean of replicate annual plot volumes",
        ),
        Line2D(
            [0], [0], color="black", marker="o", markerfacecolor="none",
            linestyle="none", markersize=8,
            label="Observed annual volume (replicate range; x = one complete plot)",
        ),
    ]
    figure, axes = plt.subplots(1, 3, figsize=(16, 5.2), sharey=True)
    for axis, treatment in zip(axes, TREATMENTS):
    ...
    save_figure(
        figure,
        figure_dir / "annual_runoff_volume_bayes_vs_ml_v3p1",
        figure_dir / "annual_obs_vs_modeled" / "annual_runoff_volume_obs_vs_modeled_v3p1",
        figure_dir
        / "annual_complete_observed"
        / "annual_runoff_volume_complete_observed_v3p1",
        layout_top=0.88,
    )
...
def plot_annual_uncertainty_supplement(
    ...
    for analyte, group in primary.groupby("Analyte", sort=False):
        ...
        save_figure(
            figure,
            supplemental_dir / f"annual_load_{safe}_full_uncertainty_v3p1",
            layout_top=0.89,
        )
...
def main() -> None:
    ...
    manifests = [
        bayes_dir / "run_manifest_bayes_v3p1_physical_event.json",
        ml_dir / "run_manifest_ml_v3p1_physical_event.json",
    ]
    ...
    expected_event_analytes = read_required(
        bayes_dir / "event_analyte_provenance_bayes_v3p1_physical_event.csv",
        "Bayesian event-analyte provenance",
    )
    observed_event_analytes = read_required(
        bayes_dir / "observed_event_analyte_ledger_v3p1_physical_event.csv",
    ...
    bayes_ledger = normalize_ledger(
        read_required(
            bayes_dir / "event_analyte_draw_ledger_bayes_v3p1_physical_event.csv",
            "Bayesian event ledger",
        ), "Bayes", "model_only",
    )
    ml_loyo = normalize_ledger(
        read_required(ml_dir / "event_analyte_draw_ledger_ml_v3p1.csv", "ML LOYO event ledger"),
    ...
    (output_dir / "run_manifest_comparison_v3p1_physical_event.json").write_text(
