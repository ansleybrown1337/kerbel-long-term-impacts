// m_stir_mogp_v2p0.stan
// v2p0 (beta_vol made analyte-specific with hierarchical pooling; residue imputation via logit-normal regression, no per-row latent residue process):
...
parameters {
  ...
  // fixed effects
  vector[A_n] z_beta_vol;
  real mu_beta_vol;
  real<lower=0> sigma_beta_vol;
...
transformed parameters {
  ...
  vector[A_n] beta_vol;  // analyte-specific volume -> concentration slopes
  ...
  beta_vol = mu_beta_vol + sigma_beta_vol * z_beta_vol;
...
model {
  ...
  b_V      ~ normal(0, 0.7);
  a_V      ~ normal(0, 0.7);
  z_beta_vol ~ std_normal();
  mu_beta_vol ~ normal(0, 0.3);
  sigma_beta_vol ~ exponential(2);
