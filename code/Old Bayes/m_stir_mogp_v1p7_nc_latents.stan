...
transformed parameters {
  ...
  vector[N] res_logit_true; // latent residue on logit scale (non-centered)
  ...
  // Non-centered residue latent process
  for (i in 1:N) {
    res_logit_true[i] = mu_res[i] + sigma_res_proc * z_res[i];
  }


  // Residue on proportion scale used as predictor in volume and concentration models
  for (i in 1:N) {
...
model {
  ...
  // ---------------------------------------------------------------------------
  // Non-centered residue latent process standard normal
  z_res ~ std_normal();
