...
```{r cmdstan model fit}
# Fit cmdstan model: m_stir_mogp_v1p7.stan
...
#mod <- cmdstan_model("m_stir_mogp_v1p7.stan")
mod <- cmdstan_model("m_stir_mogp_v1p7_nc_latents.stan")
#mod <- cmdstan_model("m_stir_mogp_v1p7p1.stan")
#mod <- cmdstan_model("m_stir_mogp_v1p7_residue.stan") # for residue as kg/m2
...
fit <- mod$sample(
  ...
  iter_warmup     = 1000,
  iter_sampling   = 1000,
  adapt_delta     = 0.99,         # higher adapt_delta for better convergence at the cost of longer sampling time
  max_treedepth   = 15,           # increase max_treedepth for complex models to avoid hitting the depth limit
  refresh         = 10,          # print progress every 10 iters
...
```{r cmdstan model fit}
saveRDS(fit, file = "../out/fit_mogp_v1p7.rds")
# fit <- readRDS("./out/fit_mogp_v1p7.rds")
...
summarize_cmdstan <- function(fit, vars = NULL, n_show = 30) {
  ...
  # v1p7 convenience: highlight crop/residue blocks if present
...
## v1p7: correct analyte × crop effects for concentration + residue-safe outputs
## =====================================================================
...
p_resC <- ggplot2::ggplot(
  ...
  ggplot2::labs(
    ...
    title = "Posterior residue effects on concentration (v1p7)",
...
ggplot2::ggsave(
  filename = "../figs/residue_effect_on_concentration_by_analyte_v1p7.png",
  plot = p_resC,
  width = 10,
  height = 7,
  dpi = 300
...
plot(
  ...
  ylab = "Density",
  main = "Posterior residue effect on volume (v1p7)",
  bty  = "l"
)
abline(v = 0, lty = 2)

jpeg("../figs/residue_effect_on_volume_density_v1p7.jpeg", width = 2000, height = 1300, res = 200)
plot(
  dens,
  type = "l",
  lwd  = 3,
...
p_cropC <- ggplot2::ggplot(
  ...
  ggplot2::labs(
    ...
    y = "Analyte",
    fill = "Effect (z)",
    title = "Posterior crop effects on concentration (v1p7)"
  ) +
...
ggplot2::ggsave(
  filename = "../figs/crop_effects_on_concentration_heatmap_v1p7.png",
  plot = p_cropC,
  width = 10.5,
  height = 6.5,
  dpi = 300
...
p_cropV <- ggplot2::ggplot(
  ...
  ggplot2::labs(
    ...
    title = "Posterior crop effects on volume (v1p7)"
  ) +
  ggplot2::theme_classic(base_size = 14)
...
ggplot2::ggsave(
  filename = "../figs/crop_effects_on_volume_v1p7.png",
  plot = p_cropV,
  width = 10,
  height = 6,
  dpi = 300
...
# Save as JPEG in figs folder (v1p7 filename)
jpeg("../figs/load1p7_STIReffects.jpeg", width = 2000, height = 1300, res = 200)
plot_stir_effects()
dev.off()
...
## v1p7: includes crop (Cr) and residue (RES) in both volume + concentration
...
simulate_load_curve_single <- function(
  ...
  irrigation = NULL,
  duplicate  = NULL,
  crop       = NULL,     # NEW v1p7: crop label (must match crop_lookup$Crop); NULL averages over crops
  res_z      = 0,        # NEW v1p7: residue (z-scale). Placeholder runs use 0.
  cin_z      = 0,
  year       = NULL,
  n_draws    = 2000,
  ci_prob    = 0.95
  ...
  # Crop (NEW v1p7): affects BOTH volume and concentration
  if (is.null(crop)) {
  ...
  # Residue (v1p7): affects BOTH volume and concentration
  # In the Stan model, residue enters on the **proportion scale** (0, 1) as:
  #   RES_true = inv_logit(res_logit_true)
  ...
  # With the v1p7 logit-normal residue submodel, interpreting 0 as a *proportion* would
  ...
  # v1p7 volume STIR params (b_V is the STIR slope; a_V is intercept)
...
###############################################################
## USER INPUT GUIDE FOR `simulate_load_curves_with_obs()` (v1p7)
## -----------------------------------------------------------
...
## -----------------------------------------------------------
## res_z (NEW v1p7: residue cover at planting, z-scale)
## -----------------------------------------------------------
## Numeric z-score. Use:
...
## ======================================================================
## Example scenario run (v1p7 defaults, averages over most factors)
## ======================================================================
...
# Save to figs folder
ggsave(
  filename = "../figs/load1p7_STIR_load_curves.jpeg",
  plot = p_load,
...
```{r year covar plot}
## ============================================================
## Year-distance covariance functions (prior vs posterior) v1p7
## ============================================================
...
# Save figure
# ---------------------------------
jpeg("../figs/year_covariance_gp_v1p7.jpeg", width = 2000, height = 1500, res = 220)
...
## v1p7 (uses post$F_year: draws × Y × A)
## ============================================================
...
# Save to figs folder (v1p7 filename)
ggsave(
  filename = "../figs/analyte_persistence_precis_v1p7.jpeg",
  plot = gg_persist,
  width = 12, height = 10, dpi = 300
)
...
```{r year effects plot}
## ============================================================
## Year effects plot: F_year by analyte (v1p7)
...
# Save to figs folder (v1p7 filename)
ggsave(
  filename = "../figs/yearly_latent_deviations_v1p7.jpeg",
  plot = gg_F,
  width = 12, height = 10, dpi = 300
)
...
# ============================================================
# v1p7: Build a prediction frame that includes "missing chemistry years"
...
#   4) NEW v1p7 fields: Crop + residue placeholder (RES_z)
#      - Crop is required for v1p7 Stan (Cr index)
#      - Residue is required for v1p7 Stan (RES predictor)
...
build_d_pred <- function(
  ...
  # ------------------------------------------------------------
  # Residue placeholder (v1p7 requires RES predictor)
...
```{r pred data table}
# ============================================================
# v1p7: Annual table from posterior predictions summed over d_pred
# Adds Crop + Residue (RES_z) effects in BOTH volume and concentration.
#
# Assumes d_pred created by build_d_pred() (v1p7) includes:
#   Crop, RES_z
# ============================================================
...
plot_annual_by_treatment_pretty <- function(
  ...
  show_points = TRUE,
  dodge = 0.20,
  prob_ci = NULL              # NEW v1p7: if annual_tbl has prob_ci column, auto-use it in ylab
) {
  metric <- match.arg(metric)

  stopifnot(all(c("Year","analyte","treatment") %in% names(annual_tbl)))

  # v1p7 tables use *_low/*_high (no fixed "95" in the name)
...
# Modeled-only (v1p7 annual_tbl_pred)
...
ggplot2::ggsave(
  filename = "../figs/TSS_annual_load_modeled_only_v1p7.png",
  plot = p_mod,
  width = 8,
  height = 5,
  dpi = 300
...
# Observed-only (v1p7 annual_tbl_obs)
...
ggplot2::ggsave(
  filename = "../figs/TSS_annual_load_observed_only_v1p7.png",
  plot = p_obs,
  width = 8,
  height = 5,
  dpi = 300
...
save_all_analyte_obs_vs_mod_plots <- function(
  annual_both,
  metric = c("load","conc","volume"),
  out_dir = file.path("../figs", "annual_obs_vs_modeled_v1p7"),
  width = 9,
  height = 5.5,
  dpi = 300,
  device = "png"
  ...
  for (i in seq_along(analytes)) {
    ...
    fname <- file.path(out_dir, paste0("annual_", metric, "_", slug, "_obs_vs_modeled_v1p7.", device))
...
# ============================================================
# v1p7: Export annual posterior predictions (and optional observed)
