...
```{r cmdstan model fit}
# Fit cmdstan model: m_stir_mogp_v1p7p2.stan
...
#mod <- cmdstan_model("m_stir_mogp_v1p7p1.stan")
mod <- cmdstan_model("m_stir_mogp_v1p7p2.stan")
...
# -------------------------------------------------------------------
# Generic helper for Gaussian-like observed vectors (C, CIN, VOL):
# Fill missing with 0. These values are ignored in the likelihood when missidx flags missing rows.
...
# Residue helper (proportion):
# (a) Validate numeric values,
# (b) clamp observed entries into (eps, 1-eps),
# (c) fill missing with 0.5 (valid interior value).
prep_miss_residue <- function(x, eps = 1e-6, fill = 0.5) {
  ...
  # Validate observed values (do not silently allow nonsense)
  if (length(obs_idx) > 0) {
  ...
  # Convert any observed values that are still in 0–100 to 0–1 (safety net)
  # This should be redundant if upstream is correct, but avoids catastrophic mis-scaling.
  if (length(obs_idx) > 0 && any(x_num[obs_idx] > 1, na.rm = TRUE)) {
    x_num[obs_idx] <- x_num[obs_idx] / 100
  }

  # If observed values are outside [0,1] even after conversion, fail fast
  if (length(obs_idx) > 0) {
  ...
  # Clamp observed values into (eps, 1-eps) for Beta likelihood validity
  if (length(obs_idx) > 0) {
    x_num[obs_idx] <- pmin(pmax(x_num[obs_idx], eps), 1 - eps)
  }

  # Fill missing with an interior placeholder; likelihood excludes missing rows via missidx
...
fit <- mod$sample(
  ...
  show_messages   = TRUE,        # surface cmdstan messages
  output_dir      = "out_cmdstanr",
  save_warmup     = TRUE,
  adapt_delta     = 0.9,        # increase adapt_delta for better convergence (fewer divergences)
  max_treedepth   = 15         # increase max_treedepth for complex models (may increase runtime, monitor for hitting max)
)
```


```{r cmdstan model fit}
saveRDS(fit, file = "./out_cmdstanr/fit_mogp_v1p7p2.rds")
# fit <- readRDS("./out_cmdstanr/fit_mogp_v1p7p2.rds")
...
summarize_cmdstan <- function(fit, vars = NULL, n_show = 30) {
  ...
  # v1p7p2 convenience: highlight crop/residue blocks if present
...
## v1p7p2: correct analyte × crop effects for concentration + residue-safe outputs
## DROP-IN REPLACEMENT: keeps sigma_Cr and stores the correct object (matrix)
## =====================================================================
...
# ---- hyperparameters / residuals
sigma_Cr_mat   <- get_param_mat(draws_mat, "sigma_Cr")     # draws × A_n  (KEEP AS MATRIX)
...
post <- list(
  ...
  gamma_Cr_V    = gamma_Cr_V_mat,   # draws × Cr_n (vol crop effect draws)
  ...
  sigma_Cr      = sigma_Cr_mat,     # FIXED: store correct object (matrix)
...
p_resC_ridge <- ggplot(post_resC_long, aes(x = beta_resC, y = analyte_abbr)) +
  ...
  labs(
    ...
    y = "Analyte",
    #title = "Posterior residue effects on concentration (v1p7p2)",
...
ggsave(
  filename = "../figs/residue_effect_on_concentration_by_analyte_v1p7p2.jpg",
  plot = p_resC_ridge,
  width = 10,
  height = 7,
  dpi = 300
...
jpeg("../figs/residue_effect_on_volume_density_v1p7p2.jpeg", width = 2000, height = 1300, res = 200)
plot(
  dens,
  type = "l",
  lwd  = 3,
...
ggsave(
  filename = "../figs/crop_effects_on_concentration_by_analyte_v1p7p2.jpg",
  plot = p_cropC_ridge,
  width = 9,
  height = 12,
  dpi = 300
...
ggsave(
  filename = "../figs/crop_effects_on_volume_v1p7p2.jpg",
  plot = p_cropV_ridge,
  width = 10,
  height = 6,
  dpi = 300
...
# Overwrites: ../figs/load1p7p2_STIReffects.jpeg
# ------------------------------------------------------------
...
ggsave(
  filename = "../figs/load1p7p2_STIReffects.jpeg",
  plot = p_stir_ridge,
  width = 9,
  height = 9,
  dpi = 300
...
# Save to file (v1p7p2 filename)
plot_stir_volume_effect(
  fit,
  param = "b_V",
  file = "../figs/1p7p2_post_STIR_effect_on_volume.jpg"
)
...
## v1p7p2: includes crop (Cr) and residue (RES) in both volume + concentration
## UPDATED: res_z interface
##   - res_z = NULL : use observed mean residue proportion from d_mod
##   - res_z = 0    : coerced to 1e-4 (near-zero fixed proportion)
##   - 0 < res_z < 1: fixed residue proportion
## NOTE: residue submodel is NOT used in this interface.
...
# NEW: empirical mean residue proportion for averaging when res_z is NULL
# Choose the residue column you actually use for the proportion-scale covariate.
# Priority: RES_input (if you created it), else RES (if already a proportion).
...
if (!is.finite(RES_bar)) stop("Could not compute RES_bar: d_mod needs RES_input or RES as a residue proportion in (0,1).")
...
simulate_load_curve_single <- function(
  ...
  irrigation = NULL,
  duplicate  = NULL,
  crop       = NULL,      # crop label; NULL averages over crops (weighted)
  res_z      = NULL,      # UPDATED: NULL -> observed mean residue; 0 -> 1e-4; (0,1) -> fixed proportion
  cin_z      = 0,
  year       = NULL,
  n_draws    = 2000,
  ci_prob    = 0.95
  ...
  # ---------------- Residue handling (UPDATED INTERFACE) ----------------
  # res_z = NULL : use observed mean residue proportion (RES_bar)
  # res_z = 0    : coerced to 1e-4
  # 0<res_z<1    : fixed residue proportion
  ...
  if (res_prop <= 0 || res_prop >= 1) {
    stop("Residue proportion must be strictly between 0 and 1. Got: ", res_prop)
  }

  # constant residue over STIR and draws
  RES_mat <- matrix(res_prop, nrow = length(draw_idx), ncol = length(STIR_z_seq))
...
###############################################################
## USER INPUT GUIDE FOR `simulate_load_curves_with_obs()` (v1p7p2)
## -----------------------------------------------------------
...
## -----------------------------------------------------------
## res_z (v1p7p2: residue cover, proportion scale 0–1)
## -----------------------------------------------------------
...
## ======================================================================
## Example scenario run (v1p7p2 defaults, averages over most factors)
## ======================================================================
...
res_scenario <- simulate_load_curves_with_obs(
  ...
  res_z        = .1,                          # avg over observed residue proportions
...
#   duplicate    = FALSE,
#   crop         = "grain corn",
#   res_z        = 0,       # coerced to 1e-4 (near-zero residue)
#   #res_z       = 0.35,    # OR fixed residue proportion (35%)
#   #res_z       = NULL,    # avg over observed residue proportions (recommended default)
#   cin_z        = 0,
#   year         = 2016,
#   ci_prob      = 0.95
# )
...
# Save to figs folder
ggsave(
  filename = "../figs/load1p7p2_STIR_load_curves.jpeg",
  plot = p_load,
...
pm_mean <- colMeans(pm_cov, na.rm = TRUE)

# ---------------------------------
# Save figure
# ---------------------------------
jpeg("../figs/year_covariance_gp_v1p7p2.jpeg", width = 2000, height = 1500, res = 220)
...
## v1p7p2 (uses post$F_year: draws × Y × A)
...
# Save to figs folder (overwrite v1p7p2 filename)
ggsave(
  filename = "../figs/analyte_persistence_precis_v1p7p2.jpeg",
...
```{r year effects plot}
## ============================================================
## Year effects plot: F_year by analyte (v1p7p2)
...
# Save to figs folder (v1p7p2 filename)
ggsave(
  filename = "../figs/yearly_latent_deviations_v1p7p2.jpeg",
  plot = gg_F,
  width = 12, height = 10, dpi = 300
)
...
# ============================================================
# v1p7p2: Build a prediction frame that includes "missing chemistry years"
...
#   4) NEW v1p7p2 fields: Crop + residue placeholder (RES_z)
#      - Crop is required for v1p7p2 Stan (Cr index)
#      - Residue is required for v1p7p2 Stan (RES predictor)
...
build_d_pred <- function(
  ...
  # ------------------------------------------------------------
  # Residue placeholder (v1p7p2 requires RES predictor)
...
```{r pred data table}
# ============================================================
# v1p7p2: Annual table from posterior predictions summed over d_pred
# Adds Crop + Residue (RES_z) effects in BOTH volume and concentration.
#
# Assumes d_pred created by build_d_pred() (v1p7p2) includes:
#   Crop, RES_z
# ============================================================
make_annual_load_table_all_treatments_pred <- function(
  ...
  # ---- NEW: drop unmapped analytes instead of stopping ----
  if (anyNA(dat$A)) {
  ...
  # ---- NEW: optionally drop rows missing c_stats (common when analytes trimmed) ----
...
plot_annual_by_treatment_pretty <- function(
  ...
  show_points = TRUE,
  dodge = 0.20,
  prob_ci = NULL              # NEW v1p7p2: if annual_tbl has prob_ci column, auto-use it in ylab
) {
  metric <- match.arg(metric)

  stopifnot(all(c("Year","analyte","treatment") %in% names(annual_tbl)))

  # v1p7p2 tables use *_low/*_high (no fixed "95" in the name)
...
# Modeled-only (v1p7p2 annual_tbl_pred)
...
ggplot2::ggsave(
  filename = "../figs/TSS_annual_load_modeled_only_v1p7p2.png",
  plot = p_mod,
  width = 8,
  height = 5,
  dpi = 300
...
# Observed-only (v1p7p2 annual_tbl_obs)
...
ggplot2::ggsave(
  filename = "../figs/TSS_annual_load_observed_only_v1p7p2.png",
  plot = p_obs,
  width = 8,
  height = 5,
  dpi = 300
...
```{r execute plot both}
# Example usage:
plot_annual_by_treatment_obs_vs_mod(annual_both, analyte_pick = "TN", metric = "load")

```

```{r save all plots}
# ------------------------------------------------------------
# Save images (v1p7p2 compatible)
# ------------------------------------------------------------
...
save_all_analyte_obs_vs_mod_plots <- function(
  annual_both,
  metric = c("load","conc","volume"),
  out_dir = file.path("../figs", "annual_obs_vs_modeled_v1p7p2"),
  width = 9,
  height = 5.5,
  dpi = 300,
  device = "jpg"
  ...
  for (i in seq_along(analytes)) {
    ...
    fname <- file.path(out_dir, paste0("annual_", metric, "_", slug, "_obs_vs_modeled_v1p7p2.", device))
...
# ============================================================
# v1p7p2: Export annual posterior predictions (and optional observed)
