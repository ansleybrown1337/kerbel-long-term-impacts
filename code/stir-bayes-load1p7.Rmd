...
# Model 1.6 (Concentration + Volume Joint Model)
...
d_mod <- d_mod %>%
  mutate(
    ...
    res_plant_z = {
      if ("residue_prop" %in% names(.)) {
        x <- as.numeric(residue_prop)
        m <- mean(x, na.rm = TRUE)
        s <- stats::sd(x, na.rm = TRUE)
        if (!is.finite(s) || s == 0) 0 else (x - m) / s
      } else if ("residue_z" %in% names(.)) {
        as.numeric(residue_z)
      } else {
        0
      }
    }
...
```{r cmdstan model fit}
# using "m_stir_mogp_v1p7.stan" in ./code folder
...
# NEW (v1p7): crop index + residue placeholder/values
Cr_vec   <- d_mod$Cr
# RES (Residue cover at planting)
# v1p7 expects RES as a proportion in (0, 1) with missingness handled via RES_missidx.
# If no residue observations are available in the dataset, we pass RES as all-NA and let Stan impute.
if ("res_plant_prop" %in% names(d_mod)) {
  RES_vec <- as.numeric(d_mod$res_plant_prop)
} else if ("RES_prop" %in% names(d_mod)) {
  RES_vec <- as.numeric(d_mod$RES_prop)
} else if ("RES" %in% names(d_mod)) {
  RES_vec <- as.numeric(d_mod$RES)
} else {
  RES_vec <- rep(NA_real_, nrow(d_mod))
}
...
# Residue helper: expects proportion in (0,1) for observed rows.
# Missing rows are imputed in Stan using RES_missidx and N_RES_miss.
impute_var_res <- function(x, eps = 1e-6) {
  missidx <- which(is.na(x))
  x_clean <- as.numeric(x)
  # Clamp observed values to (eps, 1-eps) to avoid boundary issues
  if (length(missidx) < length(x_clean)) {
    obs_idx <- setdiff(seq_along(x_clean), missidx)
    x_clean[obs_idx] <- pmin(pmax(x_clean[obs_idx], eps), 1 - eps)
  }
  # Placeholder for missing rows: 0.5 (valid interior value)
  if (length(missidx) > 0) x_clean[missidx] <- 0.5
  list(
    missidx = missidx,
    N_miss  = length(missidx),
    clean   = x_clean
...
imp_RES <- impute_var_res(RES_vec)
...
# -------------------------------------------------------------------
# 4) NEW (v1p7): pass crop + residue into cmdstan data
# -------------------------------------------------------------------
stan_dat_cmd$Cr_n <- length(unique(Cr_vec))
stan_dat_cmd$Cr   <- as.array(as.integer(Cr_vec))

# RES is handled above with missingness (N_RES_miss, RES_missidx)
