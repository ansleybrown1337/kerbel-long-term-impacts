...
def run_all(
    *,
    ...
    if not skip_residue:
        ...
        _check_file(Path("code/pipeline/merge_residue.py"), "merge_residue.py")
    ...
    _check_file(downstream_in, "WQ x STIR (with residue)" if not skip_residue else "WQ x STIR merged")
    ...
    if not skip_bayes_clean:
        ...
        cmd = [sys.executable, "code/bayes/stir_bayes_backend.py", "--in", str(downstream_in), "--out", str(wq_cleaned)]
        if debug:
            cmd.append("--debug")
