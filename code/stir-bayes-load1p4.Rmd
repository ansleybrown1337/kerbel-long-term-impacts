...
# Model 1.4 (Concentration + Volume Joint Model)
...
# ulam cannot handle multi-output GP directly, so we write a custom Stan program in next block
m_stir_nc <- ulam(
  alist(
    ##################################################
    ...
    ##################################################
    vector[Y_n]:g_year ~ multi_normal(0, K_year),
    matrix[Y_n, Y_n]:K_year <- cov_GPL2(D, etasq_year, rhosq_year, 0.01),
    
    vector[A_n]:tau_year ~ normal(0, 1),
...
mod <- cmdstan_model("m_stir_mogp.stan") # Takes about 15 minutes (850 sec) to run on AJ SUPERCPU
...
# original standardized CIN and VOL from d_mod
cin_vec <- d_mod$cin_std
vol_vec <- d_mod$volume_z

# 1. indices of missing values (1-based, as Stan expects)
stan_dat_cmd$CIN_missidx <- which(is.na(cin_vec))
stan_dat_cmd$VOL_missidx <- which(is.na(vol_vec))

# sanity check: these lengths should match the sizes hard-coded in the Stan program
length(stan_dat_cmd$CIN_missidx)  # should be 560 for your current dataset
length(stan_dat_cmd$VOL_missidx)  # should be 1637

# 2. replace NAs with a filler value (0 is fine because CIN/VOL are z-scored)
cin_clean <- cin_vec
cin_clean[is.na(cin_clean)] <- 0

vol_clean <- vol_vec
vol_clean[is.na(vol_clean)] <- 0

stan_dat_cmd$CIN <- as.numeric(cin_clean)
stan_dat_cmd$VOL <- as.numeric(vol_clean)

# now sample with cmdstanr
fit <- mod$sample(
  data = stan_dat_cmd,
  chains = 4,
  parallel_chains = 4,
...
# If you want a "rethinking-style" posterior list:
# library(rethinking)
# post <- extract.samples(fit)  # works on stanfit and cmdstanr (via method dispatch)
# str(post)
...
# extract posterior samples ULAM-style
...
jpeg("../figs/load1p4_STIReffects.jpeg", width = 2000, height = 1300, res = 200)
...
res_scenario <- simulate_load_curves_with_obs(
  ...
  analytes     = NULL,                          # all analytes
  STIR_raw_seq = seq(0, 300, length.out = 60),
  block        = 1,                          # avg over blocks
  sampler      = "ISC",                          # avg over sampler methods
  flume        = "7 V",                          # avg over flume types
  irrigation   = 1,                          # avg over irrigations
  duplicate    = FALSE,                          # avg over duplicate status
  cin_z        = 0,                             # average inflow concentration
  year         = 2015                           # specific year
)
...
# Save to figs folder
ggsave(
  filename = "../figs/load1p4_STIR_load_curves.jpeg",
  plot = p_load,
