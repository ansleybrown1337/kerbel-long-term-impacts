...
#'
#' @param path Path to wq_cleaned.csv, absolute or relative to project root.
#' @param year_max Optional upper bound on Year (e.g., 2023). Use Inf to keep all.
#' @return A tibble containing the final shared model-input data.
load_wq_stir <- function(path = "out/wq_cleaned.csv",
                         year_max = Inf) {
  
  # Accept an absolute path, or walk upward so sourcing/running from code/bayes
  # still resolves a repository-relative path correctly.
  if (!file.exists(path)) {
    current <- normalizePath(getwd(), winslash = "/", mustWork = TRUE)
    repeat {
      candidate <- file.path(current, path)
      ...
      if (identical(parent, current)) {
        stop("File '", path, "' not found from '", getwd(), "' or any parent directory.")
      }
      current <- parent
    }
  }
