...
```{r cmdstan model fit}
# Fit cmdstan model: m_stir_mogp_v1p7p3.stan
...
# Force recompile to ensure v1p7p3 is actually what runs
mod <- cmdstan_model("m_stir_mogp_v1p7p3.stan", force_recompile = TRUE)
...
```{r cmdstan model fit}
saveRDS(fit, file = "./out_cmdstanr/fit_mogp_v1p7p3.rds")
# fit <- readRDS("./out_cmdstanr/fit_mogp_v1p7p3.rds")
...
summarize_cmdstan <- function(fit, vars = NULL, n_show = 30) {
  ...
  # v1p7p3 convenience: highlight crop/residue blocks if present
...
vars_sigma <- c(
  'mu_log_sigma_analyte','tau_log_sigma_analyte','z_log_sigma_analyte'
)
...
## v1p7p3: correct analyte × crop effects for concentration + residue-safe outputs
...
p_resC_ridge <- ggplot(post_resC_long, aes(x = beta_resC, y = analyte_abbr)) +
  ...
  labs(
    ...
    y = "Analyte",
    #title = "Posterior residue effects on concentration (v1p7p3)",
...
ggsave(
  filename = "../figs/residue_effect_on_concentration_by_analyte_v1p7p3.jpg",
  plot = p_resC_ridge,
  width = 10,
  height = 7,
  dpi = 300
...
jpeg("../figs/residue_effect_on_volume_density_v1p7p3.jpeg", width = 2000, height = 1300, res = 200)
plot(
  dens,
  type = "l",
  lwd  = 3,
...
ggsave(
  filename = "../figs/crop_effects_on_concentration_by_analyte_v1p7p3.jpg",
  plot = p_cropC_ridge,
  width = 9,
  height = 12,
  dpi = 300
...
ggsave(
  filename = "../figs/crop_effects_on_volume_v1p7p3.jpg",
  plot = p_cropV_ridge,
  width = 10,
  height = 6,
  dpi = 300
...
# Overwrites: ../figs/load1p7p3_STIReffects.jpeg
# ------------------------------------------------------------
...
ggsave(
  filename = "../figs/load1p7p3_STIReffects.jpeg",
  plot = p_stir_ridge,
  width = 9,
  height = 9,
  dpi = 300
...
# Save to file (v1p7p3 filename)
plot_stir_volume_effect(
  fit,
  param = "b_V",
  file = "../figs/1p7p3_post_STIR_effect_on_volume.jpg"
)
...
## v1p7p3: includes crop (Cr) and residue (RES) in both volume + concentration
...
###############################################################
## USER INPUT GUIDE FOR `simulate_load_curves_with_obs()` (v1p7p3)
## -----------------------------------------------------------
...
## -----------------------------------------------------------
## res_z (v1p7p3: residue cover, proportion scale 0–1)
## -----------------------------------------------------------
...
## ======================================================================
## Example scenario run (v1p7p3 defaults, averages over most factors)
## ======================================================================
...
# Save to figs folder
ggsave(
  filename = "../figs/load1p7p3_STIR_load_curves.jpeg",
  plot = p_load,
...
# Save figure
# ---------------------------------
jpeg("../figs/year_covariance_gp_v1p7p3.jpeg", width = 2000, height = 1500, res = 220)
...
## v1p7p3 (uses post$F_year: draws × Y × A)
...
# Save to figs folder (overwrite v1p7p3 filename)
ggsave(
  filename = "../figs/analyte_persistence_precis_v1p7p3.jpeg",
...
```{r year effects plot}
## ============================================================
## Year effects plot: F_year by analyte (v1p7p3)
...
# Save to figs folder (v1p7p3 filename)
ggsave(
  filename = "../figs/yearly_latent_deviations_v1p7p3.jpeg",
  plot = gg_F,
  width = 12, height = 10, dpi = 300
)
...
# ============================================================
# v1p7p3: Build a prediction frame that includes "missing chemistry years"
...
#   4) NEW v1p7p3 fields: Crop + residue placeholder (RES_z)
#      - Crop is required for v1p7p3 Stan (Cr index)
#      - Residue is required for v1p7p3 Stan (RES predictor)
...
build_d_pred <- function(
  ...
  # ------------------------------------------------------------
  # Residue placeholder (v1p7p3 requires RES predictor)
...
```{r pred data table}
# ============================================================
# v1p7p3: Annual table from posterior predictions summed over d_pred
# Adds Crop + Residue (RES_z) effects in BOTH volume and concentration.
#
# Assumes d_pred created by build_d_pred() (v1p7p3) includes:
#   Crop, RES_z
# ============================================================
...
plot_annual_by_treatment_pretty <- function(
  ...
  show_points = TRUE,
  dodge = 0.20,
  prob_ci = NULL              # NEW v1p7p3: if annual_tbl has prob_ci column, auto-use it in ylab
) {
  metric <- match.arg(metric)

  stopifnot(all(c("Year","analyte","treatment") %in% names(annual_tbl)))

  # v1p7p3 tables use *_low/*_high (no fixed "95" in the name)
...
# Modeled-only (v1p7p3 annual_tbl_pred)
...
ggplot2::ggsave(
  filename = "../figs/TSS_annual_load_modeled_only_v1p7p3.png",
  plot = p_mod,
  width = 8,
  height = 5,
  dpi = 300
...
# Observed-only (v1p7p3 annual_tbl_obs)
...
ggplot2::ggsave(
  filename = "../figs/TSS_annual_load_observed_only_v1p7p3.png",
  plot = p_obs,
  width = 8,
  height = 5,
  dpi = 300
...
save_all_analyte_obs_vs_mod_plots <- function(
  annual_both,
  metric = c("load","conc","volume"),
  out_dir = file.path("../figs", "annual_obs_vs_modeled_v1p7p3"),
  width = 9,
  height = 5.5,
  dpi = 300,
  device = "jpg"
  ...
  for (i in seq_along(analytes)) {
    ...
    fname <- file.path(out_dir, paste0("annual_", metric, "_", slug, "_obs_vs_modeled_v1p7p3.", device))
...
# ============================================================
# v1p7p3: Export annual posterior predictions (and optional observed)
