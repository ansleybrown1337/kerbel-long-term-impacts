...
simulate_load_curve_single <- function(
  ...
  irrigation = NULL,
  duplicate  = NULL,
  crop       = NULL,     # NEW v1p7p1: crop label (must match crop_lookup$Crop); NULL averages over crops
  res_z      = 0,        # NEW v1p7p1: residue (z-scale). Placeholder runs use 0.
  cin_z      = 0,
  year       = NULL,
  n_draws    = 2000,
  ci_prob    = 0.95
  ...
  # Crop (NEW v1p7p1): affects BOTH volume and concentration
  if (is.null(crop)) {
  ...
  # Residue (v1p7p1): affects BOTH volume and concentration
  # In the Stan model, residue enters on the **proportion scale** (0, 1) as:
  #   RES_star = inv_logit(mu_res)
  # and is used directly in the VOL and C linear predictors.
  #
  # Historically, this Rmd treated `res_z` as a z-scale placeholder and defaulted to 0.
  # With the v1p7p1 logit-normal residue submodel, interpreting 0 as a *proportion* would
  # imply ~0% residue cover, which is usually NOT the intent and can distort predictions.
  #
  # Drop-in behavior implemented here:
  #   - If res_z is exactly 0 (the historical default) OR is NA, we use the *residue submodel*
  #     to generate an expected residue proportion as a function of STIR (and optionally crop).
  #   - If res_z is a fixed proportion in (0,1), we use that constant proportion for all STIR.
  #     (If you truly want 0 residue, pass a tiny value like 1e-6.)
  #
  # This keeps old calls working while aligning predictions with the Stan generative structure.
  if (length(res_z) != 1L || (!is.numeric(res_z) && !is.na(res_z))) {
    stop("res_z must be a single numeric value (use 0 or NA to use the residue submodel).")
  }

  # Build a draws × STIR grid of residue proportions
  use_residue_submodel <- is.na(res_z) || identical(as.numeric(res_z), 0)

  if (use_residue_submodel) {
    # Crop term in residue model: either selected crop, or average across crops
    if (is.null(crop)) {
      gamma_Cr_res_draws <- post$gamma_Cr_res[draw_idx, , drop = FALSE]  # draws × Cr_n
      gamma_Cr_res_star  <- apply(gamma_Cr_res_draws, 1, mean)           # draws
    } else {
      Cr_idx_res <- crop_lookup$Cr[crop_lookup$Crop == crop]
      if (length(Cr_idx_res) != 1L) stop("crop not found in crop_lookup")
      gamma_Cr_res_star <- post$gamma_Cr_res[draw_idx, Cr_idx_res]
    }

    # Expected residue proportion E[RES | STIR, crop] for each draw and STIR point
    res_int <- qlogis(post$res_base[draw_idx]) + gamma_Cr_res_star              # draws
    res_slope <- post$b_res_stir[draw_idx]                                      # draws
    mu_res_mat <- matrix(res_int, nrow = length(draw_idx), ncol = length(STIR_z_seq)) +
      (res_slope %o% STIR_z_seq)
    RES_mat <- plogis(mu_res_mat)
    res_label <- "model_based"
  } else {
    # Fixed proportion supplied by the user
    res_prop <- as.numeric(res_z)
    if (!is.finite(res_prop) || res_prop <= 0 || res_prop >= 1) {
      stop("If providing a fixed residue value, res_z must be a finite proportion strictly between 0 and 1.")
    }
    RES_mat <- matrix(res_prop, nrow = length(draw_idx), ncol = length(STIR_z_seq))
    res_label <- sprintf("fixed_%.3f", res_prop)
  }
  ...
  # v1p7p1 volume STIR params (b_V is the STIR slope; a_V is intercept)
  ...
  for (k in seq_len(n_x)) {
    ...
    # Residue proportion for this STIR point (vector of length = n_draws)
    RES_star <- RES_mat[, k]
  ...
  data.frame(
    ...
    crop_level       = crop_label,    residue_mode    = res_label,
    res_z            = if (use_residue_submodel) NA_real_ else as.numeric(res_z),
    res_fixed_prop  = if (use_residue_submodel) NA_real_ else as.numeric(res_z),
    cin_z            = cin_z,
...
## -----------------------------------------------------------
## res_z (NEW v1p7p1: residue cover at planting, z-scale)
## -----------------------------------------------------------
## Numeric z-score. Use:
...
```{r year covar plot}
## ============================================================
## Year-distance covariance functions (prior vs posterior) v1p7p1
## ============================================================
...
```{r execute plot both}
# Example usage:
plot_annual_by_treatment_obs_vs_mod(annual_both, analyte_pick = "TSS", metric = "load")
