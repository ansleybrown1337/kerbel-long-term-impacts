...
# Residue effect on concentration by analyte (89% HPDI)
# If residue is still a placeholder (all zeros), the effect is not identifiable; we display zeros.

res_df <- analyte_effects %>%
  dplyr::transmute(
    analyte_abbr,
    beta_resC_mean = beta_resC_eff_mean,
    beta_resC_low  = beta_resC_eff_hpdi[1, ],
    beta_resC_high = beta_resC_eff_hpdi[2, ],
    residue_placeholder
  ) %>%
  dplyr::arrange(beta_resC_mean)

p_resC <- ggplot2::ggplot(
  res_df,
  ggplot2::aes(x = reorder(analyte_abbr, beta_resC_mean), y = beta_resC_mean)
) +
  ggplot2::geom_hline(yintercept = 0, lty = 2) +
  ggplot2::geom_errorbar(
    ggplot2::aes(ymin = beta_resC_low, ymax = beta_resC_high),
    width = 0,
    linewidth = 1.0
  ) +
  ...
  ggplot2::labs(
    x = "Analyte",
    y = "Residue slope on concentration (z-scale)",
    title = "Posterior residue effects on concentration (v1p7p1)",
    subtitle = if (any(res_df$residue_placeholder)) "Residue is currently a placeholder (all zeros), effects are not identifiable" else NULL
  ) +
...
ggplot2::ggsave(
  filename = "../figs/residue_effect_on_concentration_by_analyte_v1p7p1.png",
  plot = p_resC,
  width = 10,
  height = 7,
  dpi = 300
...
plot(
  ...
  lwd  = 3,
  col  = "darkred",
  xlab = "Residue slope on volume (z-scale)",
  ylab = "Density",
  main = "Posterior residue effect on volume (v1p7p1)",
  bty  = "l",
...
# Crop effects on concentration by analyte (89% HPDI), displayed as a heatmap

crop_conc_plot_df <- crop_effects_conc %>%
  dplyr::mutate(
    analyte_abbr = factor(analyte_abbr, levels = unique(analyte_lookup$analyte_abbr)),
    Crop = factor(Crop, levels = unique(crop_lookup$Crop))
  )

p_cropC <- ggplot2::ggplot(
  crop_conc_plot_df,
  ggplot2::aes(x = Crop, y = analyte_abbr, fill = crop_eff_mean)
) +
  ggplot2::geom_tile() +
  ggplot2::labs(
    x = "Crop",
    y = "Analyte",
    fill = "Effect (z)",
    title = "Posterior crop effects on concentration (v1p7p1)"
  ) +
...
ggplot2::ggsave(
  filename = "../figs/crop_effects_on_concentration_heatmap_v1p7p1.png",
  plot = p_cropC,
  width = 10.5,
  height = 6.5,
  dpi = 300
...
# Crop effects on volume (89% HPDI) as error bars

p_cropV <- ggplot2::ggplot(
  crop_effects_vol,
  ggplot2::aes(x = reorder(Crop, crop_eff_mean), y = crop_eff_mean)
) +
  ggplot2::geom_hline(yintercept = 0, lty = 2) +
  ggplot2::geom_errorbar(
    ggplot2::aes(ymin = crop_low89, ymax = crop_high89),
    width = 0,
    linewidth = 1.0
  ) +
  ggplot2::geom_point(size = 2.3) +
  ...
  ggplot2::labs(
    x = "Crop",
    y = "Effect on volume (z-scale)",
    title = "Posterior crop effects on volume (v1p7p1)"
  ) +
  ggplot2::theme_classic(base_size = 14)
...
ggplot2::ggsave(
  filename = "../figs/crop_effects_on_volume_v1p7p1.png",
  plot = p_cropV,
  width = 10,
  height = 6,
  dpi = 300
...
# Save as JPEG in figs folder (v1p7p1 filename)
jpeg("../figs/load1p7p1_STIReffects.jpeg", width = 2000, height = 1300, res = 200)
plot_stir_effects()
dev.off()
...
plot_stir_volume_effect(
  fit,
  param = "b_V",
  file = "../figs/1p7_post_STIR_effect_on_volume.jpeg"
)
