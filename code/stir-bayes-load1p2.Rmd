...
# Model 1.2 (Concentration + Volume Joint Model)
...
m_stir_nc <- ulam(
  alist(
    ...
    transpars> matrix[A_n,4]:v_A <-
      compose_noncentered( sigma_A , L_A , Z_A ) ,
    ...
    ##################################################
    transpars> matrix[A_n,B_n]:gamma_B <-
      compose_noncentered( sigma_B , L_B , Z_B ) ,
    ...
    ##################################################
    # 5. Analyte × sampler effects (non-centered MVN)
    ##################################################
    transpars> matrix[A_n,S_n]:gamma_S <-
      compose_noncentered( sigma_S , L_S , Z_S ) ,
    ...
    ##################################################
    # 6. Analyte × flume effects (non-centered MVN)
    ##################################################
    transpars> matrix[A_n,F_n]:gamma_F <-
      compose_noncentered( sigma_F , L_F , Z_F ) ,
    ...
    # analyte-specific residual SDs for concentration
...
simulate_load_curve_single <- function(
  ...
  # Block (Rep)
  if (is.null(block)) {
    # average over all blocks for THIS analyte
    gamma_B_draws <- post$gamma_B[draw_idx, j_idx, , drop = FALSE]  # iter × 1 × B_n
  ...
  # Sampler method
  if (is.null(sampler)) {
    gamma_S_draws <- post$gamma_S[draw_idx, j_idx, , drop = FALSE]  # iter × 1 × S_n
  ...
  # Flume type
  if (is.null(flume)) {
    gamma_F_draws <- post$gamma_F[draw_idx, j_idx, , drop = FALSE]  # iter × 1 × F_n
...
compute_observed_load_points_single <- function(
  ...
  if (!is.null(block)) {
    d_sub <- d_sub %>% filter(Rep == block)
  }

  if (!is.null(sampler)) {
    d_sub <- d_sub %>% filter(SampleMethod == sampler)
  }

  if (!is.null(flume)) {
    d_sub <- d_sub %>% filter(FlumeMethod == flume)
  }

  if (!is.null(irrigation)) {
    d_sub <- d_sub %>% filter(irr_num == irrigation)
  }

  if (!is.null(duplicate)) {
    d_sub <- d_sub %>% filter(dup == as.numeric(duplicate))
  }
...
```{r load-generative-curves}
## ------------------------------------------------------------------
## Example: averaged over blocks, samplers, flumes, duplicates,
##          but FIXED irrigation = 1
## ------------------------------------------------------------------
