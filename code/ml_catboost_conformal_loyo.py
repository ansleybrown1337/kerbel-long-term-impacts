...
Leave-One-Year-Out (LOYO) cross-validation using CatBoost + manual split-conformal
prediction intervals to model:
...
-----------------------------
MAPIE's API has changed across versions, and pinned installations differ in
...
reproducibility, this script implements split conformal regression directly:
  ...
  - Split each training fold into proper-train and calibration subsets.
...
out/ml_catboost_conformal_loyo/
  ...
  (optional) wq_cleaned_ml_imputed.csv
  (optional) imputed_row_draws.csv
  (optional) models/model_logC.cbm, model_logV.cbm (+ metadata)
  (optional) feature_importance_*.csv

figs/ml_catboost_conformal_loyo/
  cv_rmse_by_year.png
  (optional) feature_importance_*.png
"""
...
def find_repo_root(start: Path) -> Path:
    """
    Walk upward to find the repo root by looking for README.md plus out/ and figs/.
    """
    cur = start.resolve()
    for _ in range(15):
    ...
    raise FileNotFoundError(
        "Could not find repo root (expected README.md and folders: out/, figs/). "
...
def safe_log1p_series(s: pd.Series) -> pd.Series:
    """
    Coerce to numeric, set negatives to NaN, then log1p.
    """
...
def coerce_bool(df: pd.DataFrame, col: str) -> pd.DataFrame:
    """
    Coerce a column to boolean if present. Common encodings are handled:
    - booleans
    - 0/1
...
def build_feature_frame(df: pd.DataFrame) -> tuple[pd.DataFrame, list[str]]:
    desired = [
        ...
        "Lab",
        "SeasonYear", "Crop",
        "Residue_PercentCover", "Residue_DryMass_kg_m2",
...
def fit_catboost_split_conformal(
    ...
    q = float(np.quantile(resid, 1 - alpha)) if resid.size else float("nan")
    return cb, q


def predict_with_pi_manual(
    cb: CatBoostRegressor, q: float, X_test: pd.DataFrame
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        type=str,
        default=None,
        help="Path to cleaned WQ table (default: <repo>/out/wq_cleaned.csv).",
    )
    ap.add_argument("--repo", type=str, default=None, help="Optional repo root path. If omitted, auto-detected.")
    ap.add_argument("--alpha", type=float, default=0.05, help="Miscoverage rate. alpha=0.05 gives ~95% PIs.")
    ap.add_argument("--draws", type=int, default=2000, help="Monte Carlo draws for annual load propagation per group.")
    ap.add_argument("--seed", type=int, default=42, help="Random seed for splitting and Monte Carlo.")
    ap.add_argument("--exclude_flagged", action="store_true", help="Drop rows with Flag or Inflow_Flag not NA/blank.")
    ap.add_argument("--calib_size", type=float, default=0.25, help="Calibration fraction for split conformal.")
    ...
    ap.add_argument("--model_subdir", type=str, default="models")
    ap.add_argument(
        "--impute_missing",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="After LOYO, refit on all observed data and impute missing Result/Volume rows. Default: True.",
    )
    ap.add_argument(
        "--impute_only",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Skip LOYO and only impute using previously saved models/metadata in --model_subdir.",
    )
    ap.add_argument("--impute_draws", type=int, default=2000, help="Draws for imputed rows. 0 disables.")
    ap.add_argument(
        "--save_models",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Save refit full-data models (.cbm) and conformal metadata. Default: True.",
    )

    # Feature importance outputs
    ap.add_argument("--feature_importance", action="store_true", help="Compute/save mean feature importance across folds.")
    ap.add_argument("--fi_topk", type=int, default=25, help="Top-K features to show in FI plot.")

    # Convenience mode
    ap.add_argument("--fast", action="store_true", help="FAST mode (fewer iterations & draws).")
    ...
    # Exclude NoRunoff rows (if present)
    df_model = df.loc[~df["NoRunoff"].fillna(False)].copy() if "NoRunoff" in df.columns else df.copy()

    # Optional exclude flagged rows
    ...
    if len(years) < 2 and not args.impute_only:
        raise ValueError("LOYO requires at least two years.")

    rng = np.random.default_rng(args.seed)

    # ------------------------------------------------------------
    # impute_only: skip LOYO and use saved full-data models/metadata
    # ------------------------------------------------------------
    if args.impute_only:
        ...
        if list(X_all.columns) != list(featC):
            raise ValueError("Feature column mismatch for logC model. Re-run without --impute_only to refit.")
        if list(X_all.columns) != list(featV):
            raise ValueError("Feature column mismatch for logV model. Re-run without --impute_only to refit.")
        ...
        if missC.any():
            ...
            out_imp.loc[missC, "Result_mg_L_pred"] = safe_expm1(pred_log)
            out_imp["Result_mg_L_pi_low"] = np.nan
            out_imp.loc[missC, "Result_mg_L_pi_low"] = safe_expm1(pred_log - qC)
            out_imp["Result_mg_L_pi_high"] = np.nan
            out_imp.loc[missC, "Result_mg_L_pi_high"] = safe_expm1(pred_log + qC)
            out_imp["q_conformal_logC"] = np.nan
        ...
        if missV.any():
            ...
            out_imp.loc[missV, "Volume_pred"] = safe_expm1(pred_log)
            out_imp["Volume_pi_low"] = np.nan
            out_imp.loc[missV, "Volume_pi_low"] = safe_expm1(pred_log - qV)
            out_imp["Volume_pi_high"] = np.nan
            out_imp.loc[missV, "Volume_pi_high"] = safe_expm1(pred_log + qV)
            out_imp["q_conformal_logV"] = np.nan
    ...
    annual_draw_rows: list[pd.DataFrame] = []
    fi_rows_C: list[pd.DataFrame] = []
    fi_rows_V: list[pd.DataFrame] = []
    ...
    for y_hold in pbar:
        ...
        if args.feature_importance:
            try:
                ...
                fi_rows_C.append(pd.DataFrame({"Year_Test": int(y_hold), "feature": list(X_train.columns), "importance": imp}))
            except Exception:
                pass
        ...
        if args.feature_importance:
            try:
                ...
                fi_rows_V.append(pd.DataFrame({"Year_Test": int(y_hold), "feature": list(X_train.columns), "importance": imp}))
            except Exception:
                pass
        ...
        # Annual load propagation (CV-valid): held-out year only
        ...
        if not paired.empty:
            C_draw_log = sample_uniform_interval(paired["loC"].to_numpy(), paired["hiC"].to_numpy(), int(args.draws), rng)
            V_draw_log = sample_uniform_interval(paired["loV"].to_numpy(), paired["hiV"].to_numpy(), int(args.draws), rng)
            ...
            for gi, gid in enumerate(unique_groups):
                ...
                annual_draw_rows.append(pd.DataFrame({
                    ...
                    "Treatment": trt,
                    "Analyte": an,
                    "Draw": np.arange(int(args.draws), dtype=int),
                    "AnnualLoad_mg": sums,
                }))
    ...
    # Optional refit on all observed data + impute missing rows
