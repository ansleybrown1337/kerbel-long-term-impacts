...
def main():
    ...
    # ------------------------------------------------------------------
    # Normalize ML summary schema to support both legacy and Bayes-matched outputs
    # ------------------------------------------------------------------
    # Legacy schema: Year, Treatment, Analyte, mean/median/low/high in mg
    # Bayes-matched schema: Year, treatment, analyte, load_mean/load_low/load_high in grams
    if "load_mean" in ml.columns:
        ml = ml.copy()
        if "treatment" in ml.columns and "Treatment" not in ml.columns:
            ml["Treatment"] = ml["treatment"]
        if "analyte" in ml.columns and "Analyte" not in ml.columns:
            ml["Analyte"] = ml["analyte"]

        # Convert g -> mg so annual_load_plots unit conversion continues to work
        ml["mean"] = pd.to_numeric(ml["load_mean"], errors="coerce") * 1e3
        ml["median"] = ml["mean"]
        ml["low"] = pd.to_numeric(ml.get("load_low"), errors="coerce") * 1e3
        ml["high"] = pd.to_numeric(ml.get("load_high"), errors="coerce") * 1e3
