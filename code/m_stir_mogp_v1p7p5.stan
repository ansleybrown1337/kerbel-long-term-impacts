// m_stir_mogp_v1p7p4.stan
// v1p7p4 (residue imputation via logit-normal regression, no per-row latent residue process):
...
transformed data {
  // Standardize irrigation count so irrigation-effect priors are on a stable scale.
  // IRR is integer in the data, but enters predictors via IRR_z (mean 0, sd 1).
  vector[N] IRR_real;
  vector[N] IRR_z;
  real irr_mean;
  real irr_sd;

  for (i in 1:N) IRR_real[i] = IRR[i];
  irr_mean = mean(IRR_real);
  irr_sd   = sd(IRR_real);
  if (irr_sd == 0) irr_sd = 1;   // guard (degenerate case)

  IRR_z = (IRR_real - irr_mean) / irr_sd;
}
