...
# Targeted, read-only residue and furrow-compaction diagnostic audit for an
# already sampled Bayes v3p3 fit.
...
script_path <- if (length(file_arg)) {
  ...
  normalizePath("code/bayes/diagnose_saved_fit_v3p3.R", winslash = "/")
}
...
fit_path <- file.path(
  repo_root, "results", "bayes", "v3p3_physical_event",
  "fit_mogp_v3p3_physical_event.rds"
)
if (!file.exists(fit_path)) stop("Saved fit not found: ", fit_path)
fit <- readRDS(fit_path)
results_dir <- dirname(fit_path)
figures_dir <- file.path(
  repo_root, "figures", "bayes", "v3p3_physical_event"
)
...
residue_variables <- c(
  "beta_res_V", "beta_tire_comp_V", "beta_res_C", "res_base", "b_res_stir",
...
residue_summary_path <- file.path(
  results_dir, "residue_parameter_diagnostics_v3p3_physical_event.csv"
)
readr::write_csv(residue_summary, residue_summary_path)

chain_variables <- c(
  "beta_res_V", "beta_tire_comp_V", "b_res_stir", "res_base",
...
chain_summary_path <- file.path(
  results_dir, "residue_chain_diagnostics_v3p3_physical_event.csv"
)
...
divergence_path <- file.path(
  results_dir, "sampler_divergences_by_chain_v3p3_physical_event.csv"
)
...
divergence_residue_path <- file.path(
  results_dir,
  "residue_volume_by_divergence_v3p3_physical_event.csv"
)
readr::write_csv(divergence_residue_summary, divergence_residue_path)

correlation_variables <- c(
  "beta_res_V", "beta_tire_comp_V", "a_V", "b_V", "beta_vol",
  "beta_vin", "sigma_V",
...
residue_volume_correlation_path <- file.path(
  results_dir,
  "residue_volume_parameter_correlations_v3p3_physical_event.csv"
)
readr::write_csv(
  residue_volume_correlations,
  residue_volume_correlation_path
)
compaction_volume_correlations <- stats::cor(
  correlation_draws[, "beta_tire_comp_V"],
  correlation_draws,
  use = "pairwise.complete.obs"
)[1, ] %>%
  tibble::enframe(
  ...
  dplyr::arrange(dplyr::desc(.data$absolute_correlation))
compaction_volume_correlation_path <- file.path(
  results_dir,
  "tire_compaction_parameter_correlations_v3p3_physical_event.csv"
)
readr::write_csv(
  compaction_volume_correlations,
  compaction_volume_correlation_path
)
...
beta_tire_comp_v_by_chain <- split(
  as.numeric(chain_draws$beta_tire_comp_V),
  chain_draws$.chain
)
chain_colors <- c("#D55E00", "#0072B2", "#009E73", "#CC79A7")
residue_volume_plot <- file.path(
  figures_dir,
  "residue_compaction_volume_chain_diagnostics_v3p3_physical_event.png"
)
grDevices::png(
  residue_volume_plot, width = 1800, height = 1600, res = 180
)
...
compaction_density_list <- lapply(
  beta_tire_comp_v_by_chain, stats::density
)
compaction_x_range <- range(vapply(
  compaction_density_list, function(x) range(x$x), numeric(2)
))
compaction_y_range <- c(
  0, max(vapply(
    compaction_density_list, function(x) max(x$y), numeric(1)
  ))
)
graphics::plot(
  NA, xlim = compaction_x_range, ylim = compaction_y_range,
  xlab = "Furrow tire-compaction effect on runoff volume",
  ylab = "Density",
  main = "Compaction posterior density by chain"
)
for (i in seq_along(compaction_density_list)) {
  graphics::lines(
    compaction_density_list[[i]], col = chain_colors[i], lwd = 2
  )
}
graphics::abline(v = 0, lty = 2)
graphics::legend(
  "topright", legend = paste("Chain", seq_along(compaction_density_list)),
  col = chain_colors, lwd = 2, bty = "n"
)

compaction_trace_matrix <- do.call(cbind, beta_tire_comp_v_by_chain)
graphics::matplot(
  compaction_trace_matrix, type = "l", lty = 1, col = chain_colors,
  xlab = "Post-warmup iteration",
  ylab = "Furrow tire-compaction effect on runoff volume",
  main = "Compaction trace by chain"
)
graphics::abline(h = 0, lty = 2)
graphics::legend(
  "topright",
  legend = paste("Chain", seq_len(ncol(compaction_trace_matrix))),
  col = chain_colors, lwd = 2, bty = "n"
)
grDevices::dev.off()
...
message(
  "Wrote tire-compaction parameter correlations: ",
  compaction_volume_correlation_path
)
