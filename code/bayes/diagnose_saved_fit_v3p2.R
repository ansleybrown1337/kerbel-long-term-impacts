...
# Targeted, read-only diagnostic audit for an already sampled Bayes v3p2 fit.
...
script_path <- if (length(file_arg)) {
  ...
  normalizePath("code/bayes/diagnose_saved_fit_v3p2.R", winslash = "/")
}
...
fit_path <- file.path(
  repo_root, "results", "bayes", "v3p2_physical_event",
  "fit_mogp_v3p2_physical_event.rds"
)
if (!file.exists(fit_path)) stop("Saved fit not found: ", fit_path)
fit <- readRDS(fit_path)
results_dir <- dirname(fit_path)
figures_dir <- file.path(
  repo_root, "figures", "bayes", "v3p2_physical_event"
)
...
residue_summary_path <- file.path(
  results_dir, "residue_parameter_diagnostics_v3p2_physical_event.csv"
)
...
chain_summary_path <- file.path(
  results_dir, "residue_chain_diagnostics_v3p2_physical_event.csv"
)
...
divergence_path <- file.path(
  results_dir, "sampler_divergences_by_chain_v3p2_physical_event.csv"
)
...
divergence_residue_path <- file.path(
  results_dir,
  "residue_volume_by_divergence_v3p2_physical_event.csv"
)
readr::write_csv(divergence_residue_summary, divergence_residue_path)

correlation_variables <- c(
  "beta_res_V", "a_V", "b_V", "beta_vol", "beta_vin", "sigma_V",
...
residue_volume_correlation_path <- file.path(
  results_dir,
  "residue_volume_parameter_correlations_v3p2_physical_event.csv"
)
readr::write_csv(
...
residue_volume_plot <- file.path(
  figures_dir, "residue_volume_chain_diagnostics_v3p2_physical_event.png"
)
grDevices::png(
