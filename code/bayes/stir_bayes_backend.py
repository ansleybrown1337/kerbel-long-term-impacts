...
def clean_wq_stir(df: pd.DataFrame) -> pd.DataFrame:
    ...
    New outputs (to move standardization out of Stan):
      - IRR_z: global z-score of numeric irrigation counts; storm labels S1/S2
        remain intact in Irrigation and receive missing IRR_z values
    ...
    # Irrigation remains a categorical event label because storm events use S1/S2.
    # Build IRR_z from a temporary numeric view below instead of destroying those
    # source-supported identifiers through numeric coercion.
    ...
    if "Irrigation" in out.columns:
        irrigation_numeric = pd.to_numeric(out["Irrigation"], errors="coerce")
        out["IRR_z"] = _zscore(irrigation_numeric)
    ...
    return out
def main() -> None:
    ap = argparse.ArgumentParser(description="Shared final model-input cleaning for Kerbel STIR x WQ data.")
    ap.add_argument("--in", dest="src", default="out/pipeline_csvs/wq_with_stir_by_season.csv",
                    help="Input merged WQ x STIR CSV.")
    ap.add_argument("--out", dest="dst", default="out/wq_cleaned.csv",
                    help="Output final cleaned CSV shared by Bayes and ML modeling.")
    ...
    print(f"[OK] Wrote shared model-input file -> {dst}")
    if args.debug:
