...
plot_stir_volume_effect(
  fit,
  file = "../figs/post_STIR_effect_on_volume.jpeg"
)
...
simulate_load_curve_single <- function(
  ...
  duplicate  = NULL,
  cin_z      = 0,
  year       = NULL,     # calendar year, e.g. 2014
  n_draws    = 2000
) {
...
```{r real data only}
# using real data
make_annual_load_table_all_treatments <- function(
  post,
  d_mod,
  years_used,
  analyte_lookup,
  ...
  miss <- setdiff(needed_cols, names(d_mod))
  if (length(miss) > 0) stop("d_mod is missing columns: ", paste(miss, collapse = ", "))
  ...
  # ---- map indices and attach backtransform stats ----
  dat <- d_mod %>%
    dplyr::transmute(
      Year, Treatment,
      analyte_abbr,
      STIR_raw = Season_STIR_toDate,
      Rep, SampleMethod, FlumeMethod,
      irr_num, dup = as.numeric(dup),
      cin_z
    ) %>%
    dplyr::left_join(
      analyte_lookup %>% dplyr::select(A, analyte_abbr),
      by = "analyte_abbr"
    ) %>%
    dplyr::left_join(
      block_lookup %>% dplyr::select(B, Rep),
      by = "Rep"
    ) %>%
    dplyr::left_join(
      sampler_lookup %>% dplyr::select(S, SampleMethod),
      by = "SampleMethod"
    ) %>%
    dplyr::left_join(
      flume_lookup %>% dplyr::select(Fu, FlumeMethod),
      by = "FlumeMethod"
    ) %>%
    dplyr::left_join(
      c_stats %>% dplyr::select(A, c_mean, c_sd) %>% dplyr::distinct(),
      by = "A"
    ) %>%
    dplyr::mutate(
  ...
  # Drop incomplete predictor rows (you can relax this if you want)
  dat <- dat %>%
    dplyr::filter(
      !is.na(STIR_z), !is.na(cin_z),
      !is.na(irr_num), !is.na(dup)
    )

  # ---- grouping keys (ALL treatments automatically) ----
  groups <- dat %>%
  ...
  # ---- main loop: group -> draw -> row predictions -> annual aggregates ----
  for (g in seq_len(nrow(groups))) {
    ...
    for (dd in seq_len(D)) {
      ...
      # Volume model on z scale
      muV_z <- post$a_V[s] + post$b_V[s] * stz
      V_raw <- V_mean + V_sd * muV_z  # L

      # Random effects and year effect
      # Ensure integer indices (avoid factors)
      A  <- as.integer(A)
      B  <- as.integer(B)
      S  <- as.integer(S)
      Fu <- as.integer(Fu)
      ...
      # Pull the per-draw slices as matrices, then elementwise index
      gB <- post$gamma_B[s, , , drop = TRUE][cbind(A, B)]     # length n_i
      gS <- post$gamma_S[s, , , drop = TRUE][cbind(A, S)]     # length n_i
      gF <- post$gamma_F[s, , , drop = TRUE][cbind(A, Fu)]    # length n_i
      kY <- post$F_year[s, , , drop = TRUE][cbind(Y, A)]      # length n_i (note order Y, A)


      # Concentration model on z scale
      muC_z <- post$alpha[s, A] +
        post$beta_stir[s, A] * stz +
        post$beta_cin[s, A]  * cin +
        post$beta_vol[s]     * muV_z +
        post$beta_irr[s]     * irr +
        post$beta_dup[s, A]  * dup +
        gB + gS + gF + kY

      C_raw <- c_mean + c_sd * muC_z  # mg/L

      # annual totals
      Vsum <- sum(V_raw, na.rm = TRUE)
      Lmg  <- sum(C_raw * V_raw, na.rm = TRUE)   # mg
      Lg   <- Lmg / 1000                         # g

      V_ann[dd]   <- Vsum
      L_ann_g[dd] <- Lg
      C_fw[dd]    <- if (is.finite(Vsum) && Vsum > 0) (Lmg / Vsum) else NA_real_
    }
...
# Example:
annual_tbl <- make_annual_load_table_all_treatments(
  post = post,
  d_mod = d_mod,
...
```{r plot annual loads}
# ============================================================
# Plot annual treatment trajectories (mean + 95% intervals)
# ============================================================
# annual_tbl: output tibble like you showed (Year, analyte, treatment, *_mean, *_low95, *_high95)
# analyte_pick: e.g. "NH4"
# metric: one of c("load","conc","volume")
#
plot_annual_by_treatment <- function(
  annual_tbl,
  analyte_pick,
  ...
  d <- annual_tbl %>%
    ...
    dplyr::mutate(
      ...
      treatment = forcats::fct_drop(treatment)
    ) %>%
  ...
  p <- ggplot2::ggplot(d, ggplot2::aes(x = Year, y = .data[[y_mean]], group = treatment, color = treatment)) +
    ...
    ggplot2::geom_line(position = pd, linewidth = 1.0)

  if (show_points) {
    p <- p + ggplot2::geom_point(position = pd, size = 2.2)
  }
...
# Examples:
# plot_annual_by_treatment(annual_tbl, analyte_pick = "NH4", metric = "load")
# plot_annual_by_treatment(annual_tbl, analyte_pick = "TP",  metric = "conc")
# plot_annual_by_treatment(annual_tbl, analyte_pick = "TSS", metric = "volume")

#plot_annual_by_treatment(annual_tbl, analyte_pick = "TSS",  metric = "load")
plot_annual_by_treatment(annual_tbl_pred, analyte_pick = "TSS",  metric = "load")
