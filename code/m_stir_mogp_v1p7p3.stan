// m_stir_mogp_v1p7p3.stan
// v1p7p3: partial pooling for analyte process SD (sigma_analyte[A]) on log scale; keeps latent process + global obs SD; fixes CIN prior (from v1p7p2).
...
// v1p7p3: partial pooling of analyte-specific process SD for C_true (z scale)
real mu_log_sigma_analyte;                 // group mean on log scale
real<lower=0> tau_log_sigma_analyte;       // group SD on log scale
vector[A_n] z_log_sigma_analyte;           // non-centered deviations
real<lower=0> sigma_V;                     // process SD for V_true around mu_V
               // process SD for V_true around mu_V
...
// ---------------------------------------------------------------------------
  ...
  real<lower=0> sigma_C_obs;   // global conc measurement SD (z scale)
  real<lower=0> sigma_VOL_obs;       // volume measurement SD (z scale)
}

transformed parameters {
  // v1p7p3: analyte-specific process SDs with partial pooling (z scale)
  ...
  // partial pooling construction for sigma_analyte (lognormal on z scale)
  sigma_analyte = exp(mu_log_sigma_analyte + tau_log_sigma_analyte * z_log_sigma_analyte);
...
if (N_RES_miss > 0) {
  ...
  for (i in 1:N) {
    C_true[i] = mu_C[i] + sigma_analyte[A[i]] * z_C[i];
  }
...
model {
  ...
  sigma_V       ~ exponential(2);// v1p7p3: hierarchical prior on analyte-specific process SDs (log scale)
z_log_sigma_analyte ~ normal(0, 1);
// Prior center: exp(mu_log_sigma_analyte) ~ 0.5 on z-scale (adjust if needed)
mu_log_sigma_analyte ~ normal(log(0.5), 0.75);
tau_log_sigma_analyte ~ exponential(2);
...
// ---------------------------------------------------------------------------
  ...
  // C_true is defined in transformed parameters as: C_true[i] = mu_C[i] + sigma_analyte[A[i]] * z_C[i]
  // ---------------------------------------------------------------------------
  z_C ~ std_normal();
  ...
  for (i in 1:N) {
    ...
    if (is_C_miss[i] == 0) {
      target += normal_lpdf(C[i] | C_true[i], sigma_C_obs);
    }
  }
}
...
generated quantities {
  ...
  for (i in 1:N) {
    ...
    C_rep[i]   = normal_rng(C_true[i], sigma_C_obs);
